...
import xin.harrison.im.dispatcher.MessageDispatcher;
...
public class WebSocketServerHandler extends SimpleChannelInboundHandler<WebSocketFrame> {
    ...
    private void messageHandler(ChannelHandlerContext ctx, WebSocketFrame msg) throws IOException {
        ...
        if (msg instanceof TextWebSocketFrame) {
            ...
            try {
                xin.harrison.im.bean.Message parsed = xin.harrison.im.utils.JsonUtils.fromJson(text, xin.harrison.im.bean.Message.class);
                if (parsed == null || parsed.getType() == null) {
                    ctx.writeAndFlush(new TextWebSocketFrame("Invalid message format"));
                    return;
                }
                // 传原始 JSON 给分发器，处理器里需要内容可自行解析
                MessageDispatcher.dispatch(ctx, parsed.getType(), text);
            } catch (Exception ex) {
                log.error("Failed to parse message", ex);
                ctx.writeAndFlush(new TextWebSocketFrame("Bad request: invalid JSON"));
            }
